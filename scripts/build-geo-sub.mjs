#!/usr/bin/env node
// 시·구 지도 생성 (창원·청주·이천 + 그 구). 원본: scripts/_provinces.json, _muni.json (gitignore)
...
const prov = JSON.parse(fs.readFileSync(path.join(ROOT, 'scripts', '_provinces.json'), 'utf8')).features;
const muni = JSON.parse(fs.readFileSync(path.join(ROOT, 'scripts', '_muni.json'), 'utf8')).features;
